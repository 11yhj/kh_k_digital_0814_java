package 문자열다루기;
// 자바는 문자열을 나타내는 자료형 String이며, 참조 타입 자료형 입니다.
// equals : 두개의 문자열이 동일한지 비교하여 결과값을 리턴함(truw / false), == 문자열 주소가 같은지를 물어봄
// index0f : 문자열에서 특정 문자가 시작되는 인덱스를 반환
// contains : 문자열에서 특정 문자열이 포함되어 있는지 여부 확인
// charAt : 문자열에서 특정 위치의 문자를 반환
// replaceAll : 문자열 중에서 특정 문자열을 다른 문자열로 변경 (정규식)
// substring : 문자열 중 특정 부분을 뽀아 낼 때 사용 (시작 인덱스만 주는 경우와 시작과 끝을 주는 경우가 있음)
// toUpperCase / toLowerCase : 문자열 전부를 대문자/ 문자열 전부를 소문자 변경
// trim : 문자열 앞뒤의 공백을 잘라냄
// split : 문자열을 특정 구분자로 분리하는 메소드
// toCharArry() : 문자열을 문자 배열로 변환
public class StringMethod {
    public static void main(String[] args) {
        ...
        System.out.println(a.indexOf("lt")); // 문자가 시작되는 인데스 반환
